...
public class food {
    ...
    public void insertFood(obj_food objFood) {
        ContentValues cv = new ContentValues();
        cv.put(_FOOD_NAME, objFood.get_FOOD_NAME());
        cv.put(_ID_PLAN, objFood.get_ID_PLAN());
        db.insert(TABLE_FOOD, null, cv);

    }

    public int updateFood(obj_food objFood) {
        ContentValues cv = new ContentValues();
        cv.put(_ID_FOOD, objFood.get_ID_FOOD());
        cv.put(_FOOD_NAME, objFood.get_FOOD_NAME());
        cv.put(_ID_PLAN, objFood.get_ID_PLAN());
        db.insert(TABLE_FOOD, null, cv);
        return db.update(TABLE_FOOD, cv, _ID_FOOD + " = ? ", new String[]{String.valueOf(objFood.get_ID_FOOD())});
    }
}
